...
public class SemanticHighlightingReconciler {
		...
		public boolean visit(ConstructorInvocation node) {
			if (fJobDeprecatedMemberHighlighting != null) {
				...
				if (constructorBinding != null
					...
					if (offset >= 0) {
						addPosition(offset, length,
								fJobDeprecatedMemberHighlighting);
					}
				}
			}
			return true;
	 ...
	 */
	public void install(ATGEditor editor, ISourceViewer sourceViewer,
		...
		synchronized (fJobLock) {
			...
			fJob = new Job("SemanticHighlighting_job") {
				...
				protected IStatus run(IProgressMonitor monitor) {
					...
					synchronized (fJobLock) {
						// allow garbage collection on job
						if (fJob == this) {
							fJob = null;
						}
					}
